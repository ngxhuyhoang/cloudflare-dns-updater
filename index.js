...
async function main() {
  try {
    ...
    // Get Zone ID
    const cfZoneIdReqUrl = `https://api.cloudflare.com/client/v4/zones?name=${encodeURI(
      `${config.hostname.split(".").reverse()[1]}.${
        config.hostname.split(".").reverse()[0]
      }`
    )}`;
    const cfZoneIdRes = await axios.get(cfZoneIdReqUrl, {
      headers: cfAuthHeaders,
    });
    if (cfZoneIdRes.data.result.length <= 0) {
    ...
    results.forEach((result) => {
      if (!result || !result.data) {
        console.error(
          `Warning: null result received, see above for error messages`
        );
        return;
      }
